...
### 2. Updated Edge Function: `supabase/functions/printful-catalog/index.ts`
Replace your existing Edge Function code with this updated version. It correctly calculates the raw signature bytes, securely supports `GET` request body structures, and provides a wildcard CORS proxy for both manufacturers.
...
async function apliiqHeaders(method: string, bodyObj?: any): Promise<Record<string, string>> {
  ...
  if (bodyObj && (method === "POST" || method === "PUT")) {
    ...
    reqBodyB64 = btoa(text);
  }
  
  const rawData = `${APLIIQ_APP_KEY}${rts}${state}${reqBodyB64}`;
...
async function apliiqFetch(method: string, path: string, bodyObj?: any): Promise<any> {
  ...
  const headers = await apliiqHeaders(method, bodyObj);
...
Deno.serve(async (req) => {
  ...
  // Admin access validation is required for standard catalog actions
