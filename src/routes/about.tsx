...
function About() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const r = heroContainerRef.current.getBoundingClientRect();
      const vh = window.innerHeight;
      const raw = 1 - r.bottom / (vh + r.height);
      setHeroScrollP(Math.max(0, Math.min(1, raw)));
    };
  ...
  useEffect(() => {
    ...
    if (heroContainerRef.current) obs.observe(heroContainerRef.current);
  ...
  useEffect(() => {
    const handleRotatorScroll = () => {
      ...
      const rect = rotatorContainerRef.current.getBoundingClientRect();
      const vh = window.innerHeight;
      const progress = -rect.top / (rect.height - vh);
      const clamped = Math.max(0, Math.min(0.99, progress));
      const targetIndex = Math.floor(clamped * 6);
    ...
    <div className="about-page w-full bg-black text-white" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui" }}>
      ...
      <section ref={heroContainerRef} id="overview" className="relative flex min-h-screen items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-20 mx-auto flex max-w-[1200px] flex-col items-center px-6 pt-24 text-center">
          ...
          <h1 className="text-balance text-white" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui", fontWeight: 600, fontSize: "clamp(54px, 8.5vw, 114px)", lineHeight: 0.98, letterSpacing: "-0.04em", opacity: 1 - heroScrollP * 1.5 }}>
            Built for those<br />who notice.
          </h1>
          <p className="mt-6 max-w-2xl text-balance text-[16px] font-light leading-relaxed text-white/70 sm:text-[19px]" style={{ opacity: 1 - heroScrollP * 1.5 }}>
            ...
            <section key={idx} ref={featRefs[idx]} className="relative overflow-hidden py-24 sm:py-36 border-b border-white/5 bg-black">
              <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-16 px-6 md:grid-cols-2" style={{ opacity: isVisible ? 1 : 0, transform: isVisible ? "translateY(0)" : "translateY(32px)", transition: "opacity 0.9s cubic-bezier(.16,1,.3,1), transform 0.9s cubic-bezier(.16,1,.3,1)" }}>
                <div className={feat.side === "right" ? "md:order-2" : ""}>
                  ...
                  <p className="mt-5 max-w-md text-[14px] font-light leading-relaxed text-neutral-400">{feat.body}</p>
                </div>
            ...
            <div className="flex gap-1.5">
              {hatAngles.map((_, idx) => (
                <button key={idx} onClick={() => {
                  const targetScroll = rotatorContainerRef.current ? rotatorContainerRef.current.offsetTop + (idx / hatAngles.length) * (rotatorContainerRef.current.offsetHeight - window.innerHeight) : 0;
