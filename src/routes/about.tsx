...
function About() {
  ...
  return (
    <div className="about-page w-full bg-background text-foreground selection:bg-neutral-800 transition-colors duration-300" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui" }}>
      
      {/* ───────────────────────────────────────────────────────────────────
          1. CINEMATIC PRODUCT HERO (SHIRT EXPLODED VIEW)
          ─────────────────────────────────────────────────────────────────── */}
      <section ref={heroRef} id="shirt-hero" className="grid grid-cols-1 md:grid-cols-2 border-b border-border min-h-[88vh]">
        
        {/* LEFT — Seamless Stage Visualizer */}
        <div className="relative w-full h-full bg-neutral-50/40 dark:bg-neutral-950/20 overflow-hidden flex flex-col justify-between border-b md:border-b-0 md:border-r border-border" style={{ minHeight: "65vh" }}>
          ...
          <div className="relative z-20 pb-6 w-full flex flex-col items-center gap-3">
            <div className="flex p-0.5 rounded-full bg-neutral-200/50 dark:bg-neutral-900/50 border border-border backdrop-blur-sm">
              <button onClick={() => setHeroActiveTab("flat")} className={`px-4 py-1.5 text-[10px] font-mono tracking-wider uppercase rounded-full transition-all duration-300 ${heroActiveTab === "flat" ? "bg-background text-foreground shadow-sm" : "text-neutral-500 hover:text-foreground"}`}>Flat Layout</button>
              <button onClick={() => setHeroActiveTab("model")} className={`px-4 py-1.5 text-[10px] font-mono tracking-wider uppercase rounded-full transition-all duration-300 ${heroActiveTab === "model" ? "bg-background text-foreground shadow-sm" : "text-neutral-500 hover:text-foreground"}`}>On Model</button>
            </div>
            
            <span style={{ fontFamily: "monospace", fontSize: "8.5px", letterSpacing: "0.3em", color: "var(--color-muted-foreground)", textTransform: "uppercase" }} className="opacity-40">Signature GZ R-01 Tee</span>
          </div>
        </div>
          ...
          <div className="reveal-on-scroll transition-all duration-[900ms] ease-out opacity-0 translate-y-6 [&.revealed]:opacity-100 [&.revealed]:translate-y-0">
            ...
            <div className="grid grid-cols-2 mb-10 border-t border-b border-border py-4">
              {[
                ...
                <div key={i} className="py-4 pr-6" style={{ borderRight: i % 2 === 0 ? "1px solid var(--border)" : "none", borderBottom: i < 2 ? "1px solid var(--border)" : "none" }}>
      ...
      {/* ───────────────────────────────────────────────────────────────────
          2. THE DETAILS BLUEPRINTS GRID (INLINED FEATURES LOOP)
          ─────────────────────────────────────────────────────────────────── */}
      <div id="anatomy">
            ...
            <section key={idx} ref={featRefs[idx]} className="relative overflow-hidden py-24 sm:py-36 border-b border-border bg-background">
              <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-16 px-6 md:grid-cols-2" style={{ opacity: isVisible ? 1 : 0, transform: isVisible ? "translateY(0)" : "translateY(32px)", transition: "opacity 0.9s cubic-bezier(.16,1,.3,1), transform 0.9s cubic-bezier(.16,1,.3,1)" }}>
                <div className={feat.side === "right" ? "md:order-2" : ""}>
                  ...
                  <h2 className="text-foreground tracking-tighter" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui", fontSize: "clamp(34px, 4.8vw, 64px)", lineHeight: 1.04, fontWeight: 200 }}>{feat.title}</h2>
                ...
                <div className={feat.side === "right" ? "md:order-1" : ""}>
                  {/* Clean, containerless floating image display */}
                  <div className="relative flex items-center justify-center w-full min-h-[300px] md:min-h-[440px] p-6 z-10">
                    <img 
                      src={feat.src} 
                      alt={feat.title} 
                      draggable={false} 
                      className="max-h-[92%] max-w-[92%] object-contain select-none transition-transform duration-100" 
                      style={{ 
      ...
      {/* ───────────────────────────────────────────────────────────────────
          3. THE 360° SCROLL ROTATOR (HAT PORTRAIT - INLINED TRIGGERS)
          ─────────────────────────────────────────────────────────────────── */}
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative w-full aspect-square max-w-[340px] flex items-center justify-center">
              ...
              {hatAngles.map((angle, idx) => (
                <img key={idx} src={angle.src} alt={`Hat angle - ${angle.name}`} className="absolute max-h-[85%] max-w-[85%] object-contain select-none transition-opacity duration-300" style={{ opacity: hatAngleIndex === idx ? 1 : 0, visibility: hatAngleIndex === idx ? "visible" : "hidden", filter: "drop-shadow(0 30px 60px rgba(0,0,0,0.15))" }} draggable={false} />
              ))}
            </div>
          </div>
          ...
          <div className="absolute bottom-28 left-6 right-6 flex justify-center text-center z-20">
            <div className="max-w-xs relative h-10 w-full">
              {[
                ...
                <p key={idx} className="absolute inset-x-0 top-0 text-xs text-muted-foreground font-light transition-all duration-500 leading-relaxed font-sans" style={{ opacity: hatAngleIndex === idx ? 0.95 : 0, transform: hatAngleIndex === idx ? "translateY(0)" : "translateY(12px)" }}>
                  {text}
                </p>
              ))}
            </div>
      ...
      <section className="bg-background px-6 py-40 border-b border-border">
        <div className="mx-auto max-w-[900px] text-center transition-all duration-[900ms] ease-out opacity-0 translate-y-6 [&.revealed]:opacity-100 [&.revealed]:translate-y-0 reveal-on-scroll">
          ...
          <h2 className="text-foreground tracking-tighter" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui", fontSize: "clamp(38px, 6vw, 84px)", lineHeight: 1.04, fontWeight: 200 }}>
          ...
          <div className="mt-16 grid grid-cols-2 gap-8 text-left sm:grid-cols-4 border-t border-border pt-12">
            {[
              ...
              <div key={l}>
                <div className="text-foreground tracking-tighter" style={{ fontFamily: "'SF Pro Display', -apple-system, system-ui", fontSize: "clamp(36px, 4vw, 56px)", fontWeight: 200 }}>{n}</div>
      ...
      <section ref={editorialRef} id="editorial-parallax" className="relative overflow-hidden border-b border-border" style={{ height: "clamp(340px, 48vw, 620px)" }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6" style={{ background: "linear-gradient(rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.45) 100%)" }}>
          <div className="motion-reduce:opacity-100 motion-reduce:transform-none transition-all duration-[900ms] ease-out opacity-0 translate-y-6 [&.revealed]:opacity-100 [&.revealed]:translate-y-0 reveal-on-scroll">
            <p className="uppercase mb-3 font-semibold font-mono" style={{ fontSize: "9px", letterSpacing: "0.32em", color: "rgba(255,255,255,0.85)" }}>Luveni Core Systems · {new Date().getFullYear()}</p>
