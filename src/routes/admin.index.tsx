...
function AdminPage() {
  ...
  const [section, setSection] = useState<
    "overview" | "products" | "orders" | "leads" | "settings" | "site"
  >("overview");
  ...
  const convRate =
    activeOrders.length > 0 ? Math.round((paidOrders.length / activeOrders.length) * 100) : 0;
...
=======
  <div className={`min-h-screen ${isDark ? "bg-black text-white" : "bg-white text-black"}`}>
    {/* NAVBAR */}
    <nav
      className={`sticky top-0 z-50 md:border-b-0 ${isDark ? "md:bg-black md:border-0 border-b border-white/10 bg-black" : "md:bg-white md:border-0 border-b border-gray-100 bg-white"}`}
    >
      <div className="flex items-center justify-between px-6 py-4">
  ...
  }
  {
    mobileMenuOpen && (
      <div
        className={`fixed inset-0 z-[100] flex flex-col items-center justify-center ${isDark ? "bg-black" : "bg-white"}`}
      >
      ...
      <div className="space-y-12">
              ...
              className={`text-[9px] font-bold uppercase px-4 py-2 transition-all ${
                revenueRange === r
                  ...
                  : isDark
                    ...
                    : "border border-gray-200 text-black/50 hover:text-black"
              }`}
            >
              {r}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <Stat
            ...
            sub={`${revenueRange.toUpperCase()} RANGE`}
            isDark={isDark}
          />
          <Stat
            label="Orders"
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2
              className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? "text-white/50" : "text-gray-400"}`}
            >
              Recent Orders
            </h2>
            <button
...
>>>>>>> 44d09e9 (fix: resolve type errors and stabilize build)
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-2 h-2 rounded-full opacity-0 group-hover:opacity-100 transition-all ${isDark ? "bg-white" : "bg-black"}`}
                  />
                  <div>
                    <p className="text-xs font-bold uppercase">{o.name || o.email}</p>
                    <p
                      className={`text-[9px] uppercase ${isDark ? "text-white/50" : "text-gray-400"}`}
                    >
...
=======
      ...
      <div className="space-y-8">
          ...
          <input
            ...
            className={`text-[10px] font-bold uppercase border-b focus:outline-none pb-1 w-48 bg-transparent ${
              isDark
                ...
                : "border-black text-black placeholder-black/30"
            }`}
          />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr
                className={`text-[9px] font-bold uppercase tracking-widest border-b ${
              ...
              {filteredOrders.map((o) => (
                <tr
                  ...
                  className={`group hover:cursor-pointer transition-all ${
                    isDark
                      ? "hover:bg-white/5 divide-white/10"
                      : "hover:bg-gray-50/50 divide-gray-50"
                  }`}
                >
                  <td className="py-6">
                  ...
                  <td className="py-6">
                    <span
                      className={`text-[9px] font-bold uppercase px-2 py-1 rounded-full border ${STATUS_CONFIG[o.status]?.color || (isDark ? "text-white/50 border-white/10" : "text-gray-400 border-gray-100")}`}
                    >
                      {o.status}
                    </span>
                  </td>
                  <td
                    className={`py-6 text-[10px] uppercase ${isDark ? "text-white/50" : "text-gray-400"}`}
                  >
    ...
    {section === "products" && (
      <div className="space-y-12">
        ...
        {productFormOpen && (
          <div
            className={`p-8 space-y-8 animate-in slide-in-from-top duration-300 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}
          >
            <h2
              className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? "text-white/50" : "text-gray-400"}`}
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <Input
                ...
                onChange={(v) => setProductForm((f) => ({ ...f, title: v }))}
                isDark={isDark}
              />
              <Input
                label="Price (USD)"
                value={productForm.price_cents}
                onChange={(v) => setProductForm((f) => ({ ...f, price_cents: v }))}
                type="number"
                isDark={isDark}
              />
              <Input
                ...
                onChange={(v) => setProductForm((f) => ({ ...f, slug: v }))}
                isDark={isDark}
              />
              <Input
                label="Source URL"
                value={productForm.source_url}
                onChange={(v) => setProductForm((f) => ({ ...f, source_url: v }))}
                isDark={isDark}
              />
>>>>>>> 44d09e9 (fix: resolve type errors and stabilize build)
            </div>
            <Input
              ...
              onChange={(v) => setProductForm((f) => ({ ...f, image_url: v }))}
              isDark={isDark}
            />
            <div className="space-y-2">
              <label
                className={`text-[9px] font-bold uppercase ${isDark ? "text-white/50" : "text-gray-400"}`}
              >
                Description
              </label>
              <textarea
...
>>>>>>> 44d09e9 (fix: resolve type errors and stabilize build)
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`text-[10px] font-bold uppercase ${isDark ? "text-white/50 hover:text-white" : "text-gray-400 hover:text-black"}`}
                >
                  Cancel
                </button>
                <button
            ...
            <div key={p.id} className="group relative">
              ...
              >
                {p.image_urls?.[0] ? (
                  ...
                  <span
                    className={`text-[7px] uppercase tracking-[0.3em] ${isDark ? "text-white/20" : "text-black/20"}`}
                  >
                    No Image
                  </span>
                )}
              ...
              <div className="px-2 text-center">
                <p
                  className={`mb-1 text-[9px] uppercase leading-tight tracking-[0.1em] truncate font-bold ${isDark ? "text-white" : "text-black"}`}
                >
                  {p.title}
                </p>
                <p
                  className={`text-[9px] tracking-[0.05em] ${isDark ? "text-white/70" : "text-black/70"}`}
                >
                ...
                <div className="flex items-center justify-center gap-3 mt-3">
                  ...
                  <button
                    ...
                    className={`${isDark ? "text-white/40 hover:text-white" : "text-black/30 hover:text-black"} transition-colors`}
                  >
                    <Edit3 size={12} />
                  </button>
                  <button
                    onClick={() => archiveProduct(p.id)}
                    className={`${isDark ? "text-white/40 hover:text-red-400" : "text-black/30 hover:text-red-500"} transition-colors`}
                  >
                    <Archive size={12} />
>>>>>>> 44d09e9 (fix: resolve type errors and stabilize build)
                  </button>
    ...
    {section === "site" && (
      <div className="max-w-2xl space-y-12 opacity-50">
        <h1 className="text-2xl font-bold uppercase tracking-tighter">Website Builder (Hidden)</h1>
        <p
          className={`text-[10px] uppercase tracking-widest ${isDark ? "text-white/50" : "text-gray-400"}`}
        >
          This section is currently hidden from the main menu but the code remains intact for future
          cleanup.
        </p>
          ...
          <div className="space-y-4">
            ...
            <div className={`p-6 space-y-6 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-widest">Theme</span>
                <div
                  className={`flex border overflow-hidden ${isDark ? "border-white/20" : "border-black/20"}`}
                >
                  <button
                    onClick={() => {
                      setSiteContent((s) => ({ ...s, theme: "light" }));
                      setSiteEdited(true);
                    }}
...
=======
          ...
          <div className="space-y-4">
            ...
            <div className={`p-6 space-y-4 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
              <div>
                <p className={`text-[10px] ${isDark ? "text-white/50" : "text-gray-400"}`}>
                  Signed in as
                </p>
                <p className="text-xs font-bold uppercase">{userEmail || "…"}</p>
              </div>
...
>>>>>>> 44d09e9 (fix: resolve type errors and stabilize build)
          ...
          <div className="space-y-4">
            ...
            <div className={`p-6 space-y-3 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
              ...
              <pre
                className={`text-[9px] border p-3 overflow-x-auto font-mono ${isDark ? "bg-black border-white/10" : "bg-white border-gray-200"}`}
              >
                {typeof window !== "undefined" ? window.location.origin : ""}
                /api/public/stripe-webhook
              </pre>
              <p className={`text-[9px] ${isDark ? "text-white/50" : "text-gray-400"}`}>
          ...
          <div className="space-y-4">
            ...
            <div className={`p-6 space-y-2 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
              <p className={`text-[10px] ${isDark ? "text-white/50" : "text-gray-400"}`}>
                Brand name and default copy live in{" "}
                <code
                  className={`px-1 py-0.5 text-[9px] font-mono border ${isDark ? "bg-black border-white/10" : "bg-white border-gray-200"}`}
                >
                  src/config/site.ts
                </code>
                .
  ...
  }
  {
    selectedRow && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
        ...
        />
        <div
          className={`relative w-full max-w-lg p-12 space-y-8 border max-h-[90vh] overflow-y-auto ${
            isDark ? "bg-black border-white/10" : "bg-white border-gray-100"
          }`}
        >
          <div className="flex items-center justify-between">
            <h3
              className={`text-sm font-bold uppercase tracking-widest ${isDark ? "text-white" : "text-black"}`}
            >
              Details
            </h3>
            ...
            {Object.entries(selectedRow).map(
              ([k, v]) =>
                k !== "_type" && (
                  <div
                    key={k}
                    className={`flex justify-between py-2 border-b gap-4 ${isDark ? "border-white/10" : "border-gray-50"}`}
                  >
                    <span
                      className={`text-[9px] font-bold uppercase flex-shrink-0 ${isDark ? "text-white/50" : "text-gray-400"}`}
                    >
                      {k}
                    </span>
                    <span className="text-[10px] font-bold uppercase truncate text-right">
                      {String(v)}
                    </span>
                  </div>
                ),
...
function Stat({
  ...
  return (
    <div className="space-y-1">
      <p
        className={`text-[9px] font-bold uppercase tracking-widest ${isDark ? "text-white/50" : "text-gray-400"}`}
      >
        {label}
      </p>
      <p className="text-2xl font-bold tracking-tighter">{value}</p>
      <p
        className={`text-[8px] font-bold uppercase tracking-widest ${isDark ? "text-white/30" : "text-gray-300"}`}
      >
        {sub}
      </p>
    </div>
