...
function AdminDashboard() {
  ...
  const syncData = async () => {
    setLoading(true);
    // Talking directly to Supabase - no server function imports to break the build
  ...
  if (loading) return <div className="p-20 text-center font-black uppercase tracking-[1em] animate-pulse">Syncing...</div>;

  return (
    <div className="p-8 space-y-10 max-w-7xl mx-auto">
      <div className="flex justify-between items-end border-b-4 border-black pb-4">
        <h1 className="text-5xl font-black tracking-tighter uppercase italic">Admin_Portal</h1>
        <button onClick={syncData} className="text-[10px] font-black border-2 border-black px-4 py-1 rounded-full uppercase hover:bg-black hover:text-white transition-all">Refresh</button>
      </div>
            ...
            {orders.map(o => (
              <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="p-6 text-right space-x-6 font-black uppercase italic text-[11px]">
                  <button onClick={() => updateStatus(o.id, 'paid')} className="hover:underline decoration-2 underline-offset-4">Mark_Paid</button>
                  <button onClick={() => updateStatus(o.id, 'cancelled')} className="text-red-500 hover:underline decoration-2 underline-offset-4">Cancel</button>
                </td>
              </tr>
            ))}
          </tbody>
