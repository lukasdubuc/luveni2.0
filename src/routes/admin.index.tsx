...
function AdminPage() {
  ...
  const importPrintfulProduct = async (item: PrintfulCatalogItem) => {
    ...
    try {
      ...
      if (!apiKey) throw new Error("VITE_Printful_API_Key is not set");
      ...
      const basePriceCents = variants.length > 0 ? Math.min(...variants.map(v => v.price_cents).filter(p => p > 0)) : 0;
      setProductForm(f => ({
        ...f,
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      <nav className={`sticky top-0 z-50 md:border-b-0 ${isDark ? "md:bg-black md:border-0 border-b border-white/10 bg-black" : "md:bg-white md:border-0 border-b border-gray-100 bg-white"}`}>
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex-1 flex justify-end">
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden"><Menu size={18} /></button>
        ...
        {mobileMenuOpen && (
          <div className={`fixed inset-0 z-[100] flex flex-col items-center justify-center md:hidden ${isDark ? "bg-black" : "bg-white"}`}>
            ...
            <div className="flex flex-col gap-8 text-center">
              {["overview", "products", "orders", "leads", "settings"].map(s => (
                <button key={s} onClick={() => { setSection(s as any); setMobileMenuOpen(false); }}
                  className={`text-[14px] font-bold uppercase tracking-[0.3em] transition-colors ${
                    section === s ? (isDark ? "text-white" : "text-black") : (isDark ? "text-white/50 hover:text-white" : "text-black/50 hover:text-black")
                  }`}>
                  {s}
                </button>
              ))}
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 space-y-12">
          ...
          <div className="space-y-8">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className={`text-[9px] font-bold uppercase tracking-widest border-b ${isDark ? "text-white/50 border-white/10" : "text-gray-400 border-gray-100"}`}>
                ...
                <tbody>
                  {filteredOrders.map(o => (
                    <tr key={o.id} onClick={() => setSelectedRow({ ...o, _type: "order" })}
                      className={`hover:cursor-pointer transition-all ${isDark ? "hover:bg-white/5" : "hover:bg-gray-50/50"}`}>
                      <td className="py-6">
        ...
        {section === "products" && (
          <div className="space-y-12">
            <div className="flex items-end justify-between">
              ...
              <button
                onClick={() => {
                  if (productFormOpen && !productForm.editingId) { setProductFormOpen(false); }
                  else { resetProductForm(); setProductFormOpen(true); }
                }}
            ...
            {productFormOpen && (
              <div className={`p-8 space-y-8 animate-in slide-in-from-top duration-300 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={syncFromPrintful} disabled={printfulLoading}
                    className={`text-[9px] font-bold uppercase px-5 py-2 border transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
                      isDark ? "border-white/20 hover:bg-white/10 text-white/70 hover:text-white" : "border-black/20 hover:bg-gray-100 text-black/60 hover:text-black"
                    }`}>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <button type="button" onClick={() => setProductForm(f => ({ ...f, hasVariants: !f.hasVariants }))}
                  ...
                  {productForm.hasVariants && (
                    <div className="space-y-2">
                      <label className={`text-[9px] font-bold uppercase ${isDark ? "text-white/50" : "text-gray-400"}`}>Variants JSON</label>
                      <textarea value={productForm.variantsText} onChange={e => setProductForm(f => ({ ...f, variantsText: e.target.value }))}
                ...
                <div key={p.id} className="group relative">
                  <div className={`relative flex aspect-[2/3] items-center justify-center overflow-hidden bg-transparent p-3 sm:p-4 group-hover:scale-105 transition-all duration-300 ${
                    ...
                    {(p.image_urls?.[1] ?? p.image_urls?.[0]) ? (
                      <img src={p.image_urls[1] ?? p.image_urls[0]} alt={p.title || "Product"} className="max-h-full max-w-full object-contain" />
                    ) : (
        ...
        {/* WEBSITE BUILDER - HIDDEN BUT PRESERVED */}
        {section === "site" && (
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className={`p-6 space-y-4 ${isDark ? "bg-white/5" : "bg-gray-50/50"}`}>
                  ...
                  <button onClick={handleSignOut} className={`w-full text-[10px] font-bold uppercase px-4 py-3 ${isDark ? "bg-red-500/10 text-red-400 hover:bg-red-500/20" : "bg-red-500/10 text-red-500 hover:bg-red-500/20"}`}>
                    LOGOUT
                  </button>
                </div>
              </div>
      ...
      {selectedRow && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className={`relative w-full max-w-lg p-12 space-y-8 border max-h-[90vh] overflow-y-auto ${isDark ? "bg-black border-white/10" : "bg-white border-gray-100"}`}>
            <div className="flex items-center justify-between">
            ...
            <div className="overflow-y-auto flex-1 p-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {printfulCatalog.map(item => (
                  <button key={item.id} type="button" onClick={() => importPrintfulProduct(item)} disabled={printfulLoading}
