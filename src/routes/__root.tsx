  ...
  return (
    <SiteShell>
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-black/55">The page you're looking for doesn't exist or has been moved.</p>
  ...
  return (
    <SiteShell>
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-black/55">Something went wrong on our end. Try refreshing or head back home.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <button onClick={() => { router.invalidate(); reset(); }} className="rounded-none bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Try again</button>
          <a href="/" className="rounded-none border border-black/10 bg-background px-4 py-2 text-sm font-medium hover:bg-accent">Go home</a>
        </div>
      </div>
    </SiteShell>
  );
...
function RootShell({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <head>
        <HeadContent />
        <script dangerouslySetInnerHTML={{ __html: `(function(){try{var t=localStorage.getItem('theme')||'light';document.documentElement.classList.add(t)}catch(e){}})()` }} />
      </head>
